...
class RNInjiVcRenderer: NSObject, RCTBridgeModule {
  ...
  @objc(convertSvgToPdf:withResolver:withRejecter:)
  func convertSvgToPdf(svgList: [String],
                       resolve: @escaping RCTPromiseResolveBlock,
                       reject: @escaping RCTPromiseRejectBlock) {
    guard let renderer = self.vcRenderer else {
      reject(nil, "InjiVcRenderer not initialized", nil)
      ...
      Task {
        let result = await renderer.convertSvgToPdf(svgList: svgList)
          if let base64 = result {
              resolve(base64)
          } else {
              reject("SVG_TO_PDF_ERROR", "Failed to convert SVG to PDF", nil)
          }
      }
  }
}
