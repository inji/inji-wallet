...
public class DriverManager {
	...
	private static MutableCapabilities desiredCapabilities;
	...
	public static AppiumDriver getAndroidDriver() {
		try {
			if (Boolean.parseBoolean(InjiWalletConfigManager.getproperty("browserstack_run"))) {
				...
				appiumDriver.set(
						new AndroidDriver(new URL("https://hub-cloud.browserstack.com/wd/hub"), desiredCapabilities));
			} else {
				desiredCapabilities = CapabilitiesReader.getDesiredCapabilities("androidDevice",
						"src/main/resources/DesiredCapabilities.json");
	...
	public static AppiumDriver getIosDriver() {
		try {
			if (Boolean.parseBoolean(InjiWalletConfigManager.getproperty("browserstack_run"))) {
				desiredCapabilities = BrowserStackCapabilitiesLoader.getCommonCapabilities();
				appiumDriver
						.set(new IOSDriver(new URL("https://hub-cloud.browserstack.com/wd/hub"), desiredCapabilities));
			} else {
				desiredCapabilities = CapabilitiesReader.getDesiredCapabilities("iosDevice",
						"src/main/resources/DesiredCapabilities.json");
