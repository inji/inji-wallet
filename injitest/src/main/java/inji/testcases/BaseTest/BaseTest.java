...
public abstract class BaseTest {
	...
	/**
	 * Executes a test step with ExtentReport logging and meaningful error context.
	 * @param stepDescription Description of the step (shown in report on failure)
	 * @param action The action to execute that returns a value
	 * @return The result of the action
	 */
	protected <T> T executeStep(String stepDescription, Supplier<T> action) {
		try {
			ExtentReportManager.getTest().info("Step: " + stepDescription);
			return action.get();
		} catch (Exception | AssertionError e) {
			ExtentReportManager.getTest().fail(stepDescription + " - FAILED: " + e.getMessage());
			throw new AssertionError(stepDescription + " failed", e);
		}
	}
	 ...
	 * Executes a test step with ExtentReport logging and meaningful error context.
	 * @param stepDescription Description of the step (shown in report on failure)
	 * @param action The action to execute (void)
	 */
	protected void executeStep(String stepDescription, Runnable action) {
		try {
			ExtentReportManager.getTest().info("Step: " + stepDescription);
			action.run();
		} catch (Exception | AssertionError e) {
